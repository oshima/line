describe Line::Message::Image do
  let(:args) do
    [
  ...
  let(:sendable) { Line::Message::Image.new(*args.deep_dup) }

  let(:received) { Line::Message::Image.new_with(attrs.deep_dup) }
  ...
  describe '#inspect' do
    ...
    context 'in the case of a received message' do
      it 'returns a string containing the id' do
        str = "#<Line::Message::Image id=#{attrs['id'].to_i.inspect}>"
